...
def display_form() -> None:
    ...
    with st.form(key='user_form'):
        ...
        cant_comidas = st.slider("Cantidad de Comidas al día", min_value=3, max_value=5)
...
def display_user_data(persona) -> None:
    ...
    with col1:
        st.metric(label="Edad", value=f"{persona.edad} años")
        st.metric(label="Altura", value=f"{persona.altura} cm")
        st.metric(label="Peso", value=f"{persona.peso} kg")
        st.metric(label="Género", value=persona.genero)
    with col2:
        st.metric(label="Nivel de Actividad", value=persona.actividad)
        st.metric(label="Objetivo de Comidas", value=persona.objetivo_comidas)
        st.metric(label="Cantidad de Comidas al Día", value=persona.cant_comidas)    
